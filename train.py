...
def get_args_parser():
    ...
    parser.add_argument('--imsize', default=518, type=int, help='image size')
...
def main(args,config):
    ...
    if config['adapter']:
        ...
        for name, parameter in text_adapter.named_parameters():
            parameter.requires_grad = True

        textmodel_clip = model.textmodel_clip
        for name, parameter in textmodel_clip.named_parameters():
            if 'adapter' in name:
                parameter.requires_grad = True
    ...
    text_ada_parameters = sum(p.numel() for p in model.text_adapter.parameters() if p.requires_grad)

    vl_transformer = sum(p.numel() for p in model.vl_transformer.parameters() if p.requires_grad)
    textmodel_clip = sum(p.numel() for p in model.textmodel_clip.parameters() if p.requires_grad)
    text_model_adap = sum(p.numel() for p in model.textmodel.parameters() if p.requires_grad)

    print('vis_ada_parameters:', vis_ada_parameters)
    print('text_ada_parameters:', text_ada_parameters)
    print('all_ada_parameters:', text_ada_parameters+vis_ada_parameters+textmodel_clip+text_model_adap)
    print('vl_transformer:', vl_transformer)
    print('textmodel_clip:', textmodel_clip)
    print('text_model_adap_bert:', text_model_adap)
    print('backbone_parameters:', text_model_adap+vis_ada_parameters)
    ...
    #     print(missing_keys)
     ...
     # lt
    # elif args.foundation_model_path is not None:
    #     # checkpoint = torch.load(args.foundation_model_path, map_location='cpu')
    #     # missing_keys, unexpected_keys = model_without_ddp.visumodel.load_state_dict(checkpoint['model'], strict=False)
        
    #     model_dict = model_without_ddp.visumodel.state_dict()
    #     state_dict = torch.load(args.foundation_model_path)
    #     model_dict.update(state_dict.items())
    #     missing_keys, unexpected_keys =model_without_ddp.visumodel.load_state_dict(model_dict, strict=False)

    #     print('Missing keys when loading dinov2 model:')
    #     print(missing_keys)
        ...
        missing_keys, unexpected_keys = model_without_ddp.visumodel.load_state_dict(checkpoint, strict=False)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            args, scaler,model, data_loader_train, optimizer, device, epoch, args.clip_max_norm)
        lr_scheduler.step()
